...
public class Paths {
    ...
    public static List<Node> minPath(Node start, Node end) {
        ...
        F.add(start, start.gems());
        data.put(start, new SF(null, start.gems()));
    ...
    public static int minDist(Node start, Node end) {
        /* TODO Read Piazza note Assignment A7 for ALL details. */
        Heap<Node> F= new Heap<Node>(true); // As in lecture slides
        ...
        while (F.size() != 0) {
            Node f= F.poll();
            if (f == end) return data.get(f).distance ;
            int fDist= data.get(f).distance;
     ...
     * if a path does not exist.
     * Note: The empty list is NOT "null"; it is a list with 0 elements. */
    public static List<Node> maxGem(Node start, Node end) {
        /* TODO Read Piazza note Assignment A7 for ALL details. */
        Heap<Node> F= new Heap<Node>(true); // As in lecture slides
        ...
        while (F.size() != 0) {
            ...
            for (Edge e : f.getExits()) {// for each neighbor w of f
                Node w= e.getOther(f);
                int newWdist= fDist + w.gems();//e.length;
                SF wInfo= data.get(w);
                if (wInfo == null) { //if w not in S or F
                    data.put(w, new SF(f, newWdist));
                    F.add(w, newWdist);
     ...
     * 
     */
    public static int maxGemDist(Node[] nodeArr) {
    		int dist = 0;
    		for (int i= 0; i < nodeArr.length-1; i++) {
    			dist = nodeArr[i].getEdge(nodeArr[i+1]).fuelNeeded();
    		}
        // no path from start to end
        return dist;
    }
