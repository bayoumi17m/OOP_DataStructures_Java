...
public class Canvas extends JPanel implements MouseListener, MouseMotionListener  {
    ...
    private void colorClick(Graphics2D g2d, Color c, int x, int y) {
    	// TODO #07 Implement me!
    		g2d.setColor(c);
        g2d.fillRect(x, y, toolSize, toolSize);
        repaint();
    ...
    private void colorDrag(Graphics2D g2d, Color c) {
		...
		g2d.drawLine((int) (mousePosPrev.getX()-toolSize/2), (int) (mousePosPrev.getY()-toolSize/2),
				(int) (mousePos.getX()-toolSize/2), (int) (mousePos.getY()-toolSize/2));
		repaint();
		window.setImageUnsaved();
    ...
    private void airBrush(Graphics2D g2d, Color c) {
    	// TODO #9 implement me!
	    	g2d.setColor(c);
	    	for (int i= 0; i <= toolSize/2; i++) {
	    		for (int k= 0; k <= toolSize/2; k++) {
	    			double randX= random.nextInt(toolSize);
	    			double randY= random.nextInt(toolSize);
	    			g2d.fillRect((int) (mousePos.getX()-toolSize/2+randX), 
	    					(int) (mousePos.getY()-toolSize/2+randY), 1, 1);
	    			repaint();
	    		}
	    	}
	    	repaint();
    ...
    @Override public void mousePressed(MouseEvent e) {
        ...
        if (activeTool == Tool.PENCIL) {
            ...
            g2d.setStroke(new BasicStroke(toolSize));
            if (!pointPressed) {
        			firstPoint= new Point2D.Double(mousePos.getX(),mousePos.getY());
            } else {
	        		Point2D.Double lastpoint= new Point2D.Double(mousePos.getX(),mousePos.getY());
	        		Line2D.Double drawnLine= new Line2D.Double(firstPoint,lastpoint);
	        		g2d.draw(drawnLine);
	        		repaint();
	        		window.setImageUnsaved();
        }
...
//    	        g2d.setColor(foreColor);
//              g2d.setStroke(new BasicStroke(toolSize));
//    			Point2D.Double lastpoint= new Point2D.Double(e.getX(),e.getY());
//        		Line2D.Double drawnLine= new Line2D.Double(firstPoint,lastpoint);
//        		g2d.draw(drawnLine);
        		repaint();
    		}
